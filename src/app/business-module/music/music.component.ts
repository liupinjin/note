...
import * as APlayer from 'assets/aplayer/APlayer.min.js';
...
export class MusicComponent implements OnInit {
  ...
  __construct(_v = 'netease'){
    this.site(_v);
  }

  site(_v){
    let _suppose = new Array('netease','tencent','xiami','kugou','baidu');
    let con = _suppose.filter(i => i == _v);
    this._SITE = con>0?con[0]:'netease';
  }

  cookie(_v = '')
  {
    if (_v) {
      this._TEMP['cookie']=_v;
    }
  }
  ...
  {
    if (_API['encode']) {
      _API=call_user_func_array(array($this,$API['encode']), array($API));
    }
    $BASE=$this->curlset();
    $curl=curl_init();
    if ($API['method']=='POST') {
